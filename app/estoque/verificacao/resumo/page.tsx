...
function ResumoContent() {
  ...
  const carregarRevisao = async (id: string) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Revisão carregada completa:', JSON.stringify(data, null, 2));
        console.log('Revisão:', data.revisao);
        console.log('Itens da revisão:', data.revisao?.itens);
  ...
  console.log('Revisão state:', revisao);
  ...
  const gerarPDF = (): jsPDF => {
    ...
    doc.text('Resumo da Revisão de Estoque', 105, 20, { align: 'center' });
    ...
    doc.text(`Total de itens: ${itensArray.length}`, 20, 56);
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-gray-50 pt-4">
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Resumo da Revisão de Estoque</h1>
